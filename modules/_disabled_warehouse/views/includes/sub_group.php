...
<table class="table dt-table border table-striped">
 <thead>
  ...
  <tbody>
    ...
    <tr>
        <td><?php echo _l($sub_group_type['id']); ?></td>
        <td><?php echo _l($sub_group_type['sub_group_code']); ?></td>
        <td><?php echo _l($sub_group_type['sub_group_name']); ?></td>
        <td><?php echo _l($sub_group_type['order']); ?></td>
        <td><?php if($sub_group_type['display'] == 0){ echo _l('not_display'); }else{echo _l('display');} ?></td>
        <td><?php echo _l($sub_group_type['note']); ?></td>

        <td>
            <?php if (has_permission('warehouse', '', 'edit') || is_admin()) { ?>
              <a href="#" onclick="edit_sub_group_type(this,<?php echo html_entity_decode($sub_group_type['id']); ?>); return false;" data-sub_group_code="<?php echo html_entity_decode($sub_group_type['sub_group_code']); ?>" data-name="<?php echo html_entity_decode($sub_group_type['sub_group_name']); ?>" data-order="<?php echo html_entity_decode($sub_group_type['order']); ?>" data-display="<?php echo html_entity_decode($sub_group_type['display']); ?>" data-note="<?php echo html_entity_decode($sub_group_type['note']); ?>" class="btn btn-default btn-icon"><i class="fa fa-pencil-square-o"></i>
            </a>
            <?php } ?>
