...
array_push($where, 'AND ('.db_prefix().'clients.is_supplier = '.$is_supplier.') ');

// print_r($is_supplier); exit;
...
foreach ($rResult as $aRow) {
    ...
    $url = admin_url('supplier/client/' . $aRow['userid']);
    ...
    if ($aRow['registration_confirmed'] == 0 && is_admin()) {
        $company .= ' | <a href="' . admin_url('supplier/confirm_registration/' . $aRow['userid']) . '" class="text-success bold">' . _l('confirm_registration') . '</a>';
    }
    if (!$isPerson) {
        $company .= ' | <a href="' . admin_url('supplier/client/' . $aRow['userid'] . '?group=contacts') . '">' . _l('customer_contacts') . '</a>';
    }
    if ($hasPermissionDelete) {
        $company .= ' | <a href="' . admin_url('supplier/delete/' . $aRow['userid']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
    }
    ...
    // Primary contact
    $row[] = ($aRow['contact_id'] ? '<a href="' . admin_url('supplier/client/' . $aRow['userid'] . '?contactid=' . $aRow['contact_id']) . '" target="_blank">' . $aRow['firstname'] . ' ' . $aRow['lastname'] . '</a>' : '');
    ...
    <input type="checkbox"' . ($aRow['registration_confirmed'] == 0 ? ' disabled' : '') . ' data-switch-url="' . admin_url() . 'supplier/change_client_status" name="onoffswitch" class="onoffswitch-checkbox" id="' . $aRow['userid'] . '" data-id="' . $aRow['userid'] . '" ' . ($aRow[db_prefix().'clients.active'] == 1 ? 'checked' : '') . '>
    ...
    $row[] = $toggleActive;
     $toggle_pre_supplier = '<div class="onoffswitch custom_toggle"><label class="toggleSwitch" onclick="">
        <input type="checkbox" data-switch-url="' . admin_url() . 'supplier/change_supplier_preference" name="onoffswitch" class="onoffswitch-checkbox" id="' . $aRow['userid'] . '" data-id="' . $aRow['userid'] . '" ' . ($aRow['is_preffered'] == 1 ? 'checked' : '') . '>
        <span>
            <span></span>
            <span></span>
    ...
    // For exporting
    $toggle_pre_supplier .= '<span class="hide">' . ($aRow['is_preffered'] == 1 ? _l('is_active_export') : _l('is_not_active_export')) . '</span>';
    $row[] = $toggle_pre_supplier;
