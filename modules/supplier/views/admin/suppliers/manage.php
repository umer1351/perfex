            ...
            <div class="panel_s">
               <div class="panel-body">
                  <div class="_buttons">
                     ...
                     <a href="<?php echo admin_url('supplier/client' ); ?>" class="btn btn-info mright5 test pull-left display-block">
                     <?php echo _l('new_supplier'); ?></a>
                     <a href="<?php echo admin_url('supplier/import'); ?>" class="btn btn-info pull-left display-block mright5 hidden-xs">
                     ...
                     <?php if (has_permission('items','','')) { ?>
                      <a href="<?php echo admin_url('invoice_items' ); ?>" class="btn btn-info mright5 test pull-left display-block">
                           <?php echo _l('items'); ?></a>
                     <?php }?>
                     <a href="<?php echo admin_url('supplier/all_contacts'); ?>" class="btn btn-info pull-left display-block mright5">
                     ...
                     <div class="btn-group pull-right btn-with-tooltip-group _filter_data" data-toggle="tooltip" data-title="<?php echo _l('filter_by'); ?>">
                        ...
                        <ul class="dropdown-menu dropdown-menu-left" style="width:300px;">
                           ...
                           <li>
                              ...
                              <?php echo _l('suppliers_assigned_to_me'); ?>
                              </a>
                           </li>
                           <li class="divider"></li>
                  ...
                  <?php if(has_permission('customers','','view') || have_assigned_customers()) {
                     ...
                     $query_active = $this->db->query('SELECT COUNT(*) as count FROM '.$contact_tbl.' LEFT JOIN '.$client_tbl.' ON '.$contact_tbl.'.userid = '.$client_tbl.'.userid WHERE '.$contact_tbl.'.active=1 AND '.$client_tbl.'.is_supplier= 1'. $where_summary);
                     $active = $query_active->row();
                     $query_inactive = $this->db->query('SELECT COUNT(*) as count FROM '.$contact_tbl.' LEFT JOIN '.$client_tbl.' ON '.$contact_tbl.'.userid = '.$client_tbl.'.userid WHERE '.$contact_tbl.'.active=0 AND '.$client_tbl.'.is_supplier= 1'. $where_summary);
                     $inactive = $query_inactive->row();
                     $query_like = $this->db->query('SELECT COUNT(*) as count FROM '.$contact_tbl.' LEFT JOIN '.$client_tbl.' ON '.$contact_tbl.'.userid = '.$client_tbl.'.userid WHERE '.$contact_tbl.'.last_login LIKE "'.date('Y-m-d').'%" AND '.$client_tbl.'.is_supplier= 1'. $where_summary);
                     $like = $query_like->row();
                  ...
                  <div class="row mbot15">
                     ...
                     <div class="col-md-2 col-xs-6 border-right">
                        <h3 class="bold"><?php echo total_rows(db_prefix().'clients','is_supplier=1',($where_summary != '' ? substr($where_summary,5) : '')); ?></h3>
                        <span class="text-dark"><?php echo _l('suppliers_summary_total'); ?></span>
                     </div>
                     <div class="col-md-2 col-xs-6 border-right">
                        <h3 class="bold"><?php echo total_rows(db_prefix().'clients','is_supplier=1 AND active=1'.$where_summary); ?></h3>
                        <span class="text-success"><?php echo _l('active_suppliers'); ?></span>
                     </div>
                     <div class="col-md-2 col-xs-6 border-right">
                        <h3 class="bold"><?php echo total_rows(db_prefix().'clients','is_supplier=1 AND active=0'.$where_summary); ?></h3>
                        <span class="text-danger"><?php echo _l('inactive_active_supplier'); ?></span>
                     </div>
                     <div class="col-md-2 col-xs-6 border-right">
                        ...
                        <?php
                           ...
                           if(count($contacts_logged_in_today)> 0){
                              foreach($contacts_logged_in_today as $contact){
                               $url = admin_url('supplier/client/'.$contact['userid'].'?contactid='.$contact['id']);
                        ...
                        <div class="modal-content">
                           ...
                           <div class="modal-body">
                              ...
                              <div id="bulk_change">
                                 ...
                                 <p class="text-danger"><?php echo _l('bulk_action_suppliers_groups_warning'); ?></p>
                              </div>
                           </div>
                  ...
                  <div class="checkbox">
                     ...
                     <label for="exclude_inactive"><?php echo _l('exclude_inactive'); ?> <?php echo _l('suppliers'); ?></label>
                  </div>
...
<script>
   $(function(){
      ...
      var is_supplier = "<?= $this->input->get('supplier'); ?>";

      

         $('#changetitle').html('<?php echo _l('supplier_summary'); ?>');
      

       var tAPI = initDataTable('.table-clients', admin_url+'supplier/table', [0], [0], CustomersServerParams,<?php echo hooks()->apply_filters('customers_table_default_order', json_encode(array(2,'asc'))); ?>);
   ...
   function customers_bulk_action(event) {
       ...
       if (r == false) {
           ...
           setTimeout(function(){
             $.post(admin_url + 'supplier/bulk_action', data).done(function() {
