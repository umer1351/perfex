...
class Supplier extends AdminController
{
    ...
    public function table()
    {
        ...
        $this->app->get_table_data(module_views_path(MODULE_SUPPLIER, 'tables/suppliers'));
    ...
    public function all_contacts()
    {
        if ($this->input->is_ajax_request()) {
            // $this->app->get_table_data('all_contacts');
             $this->app->get_table_data(module_views_path(MODULE_SUPPLIER, 'tables/all_contacts'));
        }
        ...
        $this->load->view('admin/suppliers/all_contacts', $data);
    }
    ...
    public function client($id = '')
    {
            ...
            if ($id == '') {
                ...
                if ($id) {
                    ...
                    if ($save_and_add_contact == false) {
                        redirect(admin_url('supplier/client/' . $id));
                    } else {
                        redirect(admin_url('supplier/client/' . $id . '?group=contacts&new_contact=true'));
                    }
                }
            } else {
        ...
        if ($group != 'contacts' && $contact_id = $this->input->get('contactid')) {
            redirect(admin_url('supplier/client/' . $id . '?group=contacts&contactid=' . $contact_id));
        }
    ...
    public function assign_admins($id)
    {
        ...
        redirect(admin_url('supplier/client/' . $id . '?tab=customer_admins'));
    }
    ...
    public function delete_customer_admin($customer_id, $staff_id)
    {
        ...
        redirect(admin_url('supplier/client/' . $customer_id) . '?tab=customer_admins');
    }
    ...
    public function delete_contact($customer_id, $id)
    {
        ...
        redirect(admin_url('supplier/client/' . $customer_id . '?group=contacts'));
    }
    ...
    public function delete($id)
    {
        ...
        if (is_array($response) && isset($response['referenced'])) {
            set_alert('warning', _l('supplier_delete_transactions_warning', _l('invoices') . ', ' . _l('estimates') . ', ' . _l('credit_notes')));
    ...
    /* change supplier preference */
    public function change_supplier_preference($id, $status){
        if ($this->input->is_ajax_request()) {
            $this->clients_model->change_supplier_preference($id, $status);
        }
    }
    ...
    public function zip_credit_notes($id)
    {
        ...
        if ($this->input->post()) {
            $this->load->library('app_bulk_pdf_export', [
                ...
                'redirect_on_error' => admin_url('supplier/client/' . $id . '?group=credit_notes'),
            ]);
    ...
    public function zip_invoices($id)
    {
        ...
        if ($this->input->post()) {
            $this->load->library('app_bulk_pdf_export', [
                ...
                'date_from'         => $this->input->post('zip-from'),
                'date_to'           => $this->input->post('zip-to'),
                'redirect_on_error' => admin_url('supplier/client/' . $id . '?group=invoices'),
            ]);
    ...
    public function zip_estimates($id)
    {
        ...
        if ($this->input->post()) {
            $this->load->library('app_bulk_pdf_export', [
                ...
                'date_from'         => $this->input->post('zip-from'),
                'date_to'           => $this->input->post('zip-to'),
                'redirect_on_error' => admin_url('supplier/client/' . $id . '?group=estimates'),
            ]);
    ...
    public function zip_payments($id)
    {
        ...
        $this->load->library('app_bulk_pdf_export', [
                ...
                'date_from'         => $this->input->post('zip-from'),
                'date_to'           => $this->input->post('zip-to'),
                'redirect_on_error' => admin_url('supplier/client/' . $id . '?group=payments'),
            ]);
    ...
    public function delete_group($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('supplier/groups'));
        }
    ...
    public function bulk_action()
    {
        ...
        if ($this->input->post('mass_delete')) {
            set_alert('success', _l('total_suppliers_deleted', $total_deleted));
        }
    }
    ...
    public function statement_pdf()
    {
        ...
        if (!has_permission('invoices', '', 'view') && !has_permission('payments', '', 'view')) {
            set_alert('danger', _l('access_denied'));
            redirect(admin_url('supplier/client/' . $customer_id));
        }
    ...
    public function send_statement()
    {
        ...
        redirect(admin_url('supplier/client/' . $customer_id . '?group=statement'));
    }
    ...
    public function statement()
    {
        ...
        $viewData['html'] = $this->load->view('admin/suppliers/groups/_statement', $data, true);
    ...
    public function delete_item($id,$client_id){
        ...
        if (!$id) {
            redirect(admin_url('supplier/client/'.$client_id));
        }
            ...
            if ($id == '') {
               ...
               if (!has_permission('items')) {
                ...
                if ($id) {
                    ...
                    //redirect(admin_url('supplier/client/'.$userid));
                    echo json_encode([
                    'success' => true,
