...
class Clients_model extends App_Model
{
     ...
     * @return array
     */
    public function get_user_issupplierornot ($customer_id)
    {
        ...
        $this->db->select('is_supplier');
        $this->db->where('userid',$contactuid[0]['userid']);
        $contactissupplier = $this->db->get(db_prefix() . 'clients')->result_array();



        // print_r( $contactissupplier[0]['is_supplier']); exit();

        return $contactissupplier[0]['is_supplier'];
    }
    ...
    public function change_supplier_preference($id, $status)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            log_activity('Change supplier preference Status Changed [ID: ' . $id . ' Status(Active/Inactive): ' . $status . ']');
    ...
    public function send_notification_customer_profile_file_uploaded_to_responsible_staff($contact_id, $customer_id)
    {
        ...
        foreach ($staff as $member) {
            ...
            if (add_notification([
                    ...
                    'link' => 'supplier/client/' . $customer_id . '?group=attachments',
                ])) {
