...
class Packages extends AdminController
{
    ...
    private function create_or_edit_package($id = '')
    {
        ...
        try {
            ...
            // Only allow create from dump in single or single_pool schme for security constraint.
            if (isset($form_data['metadata']['allow_create_from_dump'])) {
                if ($form_data['metadata']['allow_create_from_dump'] == 'yes' && !in_array($db_scheme, ['single_pool', 'single'])) {
                    throw new \Exception(_l('perfex_saas_allow_create_from_dump_unsupported_db_scheme'), 1);
                }
            }
            ...
            if ($_id) {
                ...
                set_alert('success', _l(empty($id) ? 'added_successfully' : 'updated_successfully', _l('perfex_saas_package')));

                return redirect(admin_url(PERFEX_SAAS_MODULE_NAME . '/packages/edit/' . $_id));
            }
        } catch (\Exception $e) {
