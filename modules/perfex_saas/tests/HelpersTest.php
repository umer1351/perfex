...
class HelpersTest extends PerfexSaasTest
{
    ...
    /**
     * Test for perfex_saas_is_write_query helper.
     * The helper detect if a query is write or not using string match and PHPSQLParser library
     *
     * @return void
     */
    public function testIsWriteQuery()
    {
        // Test cases with query and expected results
        $test_cases = [
            [
                'query' => "INSERT INTO customers (name, email) VALUES ('John Doe', 'john@example.com')",
                'expected_result' => true,
                'test_name' => 'Testing simple INSERT query',
            ],
            [
                'query' => "UPDATE products SET price = 10.99 WHERE id = 1",
                'expected_result' => true,
                'test_name' => 'Testing simple UPDATE query',
            ],
            [
                'query' => "DELETE FROM orders WHERE status = 'cancelled'",
                'expected_result' => true,
                'test_name' => 'Testing simple DELETE query',
            ],
            [
                'query' => "SELECT * FROM users",
                'expected_result' => false,
                'test_name' => 'Testing simple SELECT query',
            ],
            // Edge cases
            [
                'query' => "UPDATE products",
                'expected_result' => false,
                'test_name' => 'Testing UPDATE query without SET clause',
            ],
            [
                'query' => "DELETE FROM orders",
                'expected_result' => true,
                'test_name' => 'Testing DELETE query without WHERE clause',
            ],
            [
                'query' => "DROP DATABASE mydb",
                'expected_result' => true,
                'test_name' => 'Testing DROP DATABASE query',
            ],
            [
                'query' => "INSERT INTO customers VALUES (1, 'John Doe', 'john@example.com')",
                'expected_result' => true,
                'test_name' => 'Testing INSERT query with values',
            ],
            // Complex cases
            [
                'query' => "UPDATE products SET price = 10.99 WHERE id IN (SELECT product_id FROM categories WHERE category = 'electronics')",
                'expected_result' => true,
                'test_name' => 'Testing complex UPDATE query with subquery',
            ],
            [
                'query' => "DELETE FROM orders WHERE customer_id IN (SELECT id FROM customers WHERE created_at < DATE_SUB(NOW(), INTERVAL 30 DAYS))",
                'expected_result' => true,
                'test_name' => 'Testing complex DELETE query with subquery',
            ],
        ];
        ...
        foreach ($test_cases as $test_case) {
            ...
            $is_write_query = perfex_saas_is_write_query($query);

            // Run the test and compare the expected result with the actual result
            $this->CI->unit->run($is_write_query, $expected_result, $test_name, $this->customTestNote($is_write_query, $expected_result));
        }
    }
    ...
    public function testPerfexSaasAddTenantColumnToCreateQuery()
    {
        $testCases = [
            // Valid SQL Statement
            [
                ...
                'expected_result' => 'CREATE TABLE `users` (
                `id` INT(11) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
                `name` VARCHAR(100) NOT NULL,
                `email` VARCHAR(100) NOT NULL,
                `' . PERFEX_SAAS_TENANT_COLUMN . '` VARCHAR(50) NOT NULL
            )',
                'description' => 'Valid SQL Statement',
            ],
            // SQL Statement with Existing Tenant Column
            [
                'sql' => 'CREATE TABLE `users` (
                `id` INT(11) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
                `name` VARCHAR(100) NOT NULL,
                ...
                'description' => 'SQL Statement with Existing Tenant Column',
            ],
        ];

        foreach ($testCases as $testCase) {
            $result = perfex_saas_add_tenant_column_to_create_query($testCase['sql']);
