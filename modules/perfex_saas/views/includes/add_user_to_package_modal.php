    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body tw-m-0">
                ...
                <?= form_open(admin_url(PERFEX_SAAS_MODULE_NAME . '/packages/add_user_to_package'), ['id' => 'add_user_to_package_form']); ?>
                <!-- contact selection -->
                        ...
                        <select id="packageid" name="packageid" class="form-control">
                            <?php foreach (get_instance()->perfex_saas_model->packages() as $package) : ?>
                                <option value="<?= $package->id; ?>"><?= $package->name; ?> (<?= $package->price; ?>
                                    <?= get_base_currency()->name; ?>)</option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group select-placeholder">
                        <label for="clientid" class="control-label"><?php echo _l('perfex_saas_invoice_select_customer'); ?></label>
                        <select id="clientid" name="clientid" data-live-search="true" data-width="100%" class="ajax-search" data-none-selected-text="<?php echo _l('perfex_saas_dropdown_non_selected_tex'); ?>">
                        </select>
                    </div>
                    <div class="form-group">
