 <script>
  ...
  $('input[name="commodity_id"]').val("<?php echo html_entity_decode($commodity_item->id) ;?>");
   ...
   var  _table_api = initDataTable(table_inventory_stock, admin_url+'warehouse/table_inventory_stock', true, '', ProposalServerParams);
   ...
   var  _table_api = initDataTable(table_view_commodity_detail, admin_url+'warehouse/table_view_commodity_detail', true, '', ProposalServerParams);
    $.each(ProposalServerParams, function(i, obj) {
        $('select' + obj).on('change', function() {  
...
var ProposalServerParams = {
  ...
  var   table_warehouse_history = $('table.table-table_warehouse_history');
   var  _table_api = initDataTable(table_warehouse_history, admin_url+'warehouse/table_warehouse_history',true, '', ProposalServerParams);
    $.each(ProposalServerParams, function(i, obj) {
        $('select' + obj).on('change', function() {  
