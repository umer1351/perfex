...
if (!function_exists('installSuperadminEmailTemplates')) {
    ...
    function installSuperadminEmailTemplates()
    {
        if (!total_rows(db_prefix().'emailtemplates', ['slug'=>'inactive-tenant-reminder'])) {
            create_saas_email_template('Your account will be deleted soon', 'Email Template Content', 'superadmin', 'Inactive Tenant Reminder', 'inactive-tenant-reminder');
        }
...
if (!function_exists('installTenantsEmailTemplates')) {
    /**
     * Add some required email templates for tenants.
     */
    function installTenantsEmailTemplates()
    {
        if (!total_rows(db_prefix().'emailtemplates', ['slug'=>'inactive-tenant-reminder'])) {
            create_saas_email_template('Your account will be deleted soon', 'Email Template Content', 'tenants', 'Inactive Tenant Reminder', 'inactive-tenant-reminder');
        }
        if (!total_rows(db_prefix().'emailtemplates', ['slug'=>'onboarding-email'])) {
            create_saas_email_template('Onboarding Email', 'Email Template Content', 'tenants', 'Onboarding Email', 'onboarding-email');
        }
    }
}
...
if (!function_exists('getSaasPlans')) {
    ...
    {
        get_instance()->load->model(SUPERADMIN_MODULE.'/superadmin_model');
...
if (!function_exists('getStatsForSuperadmin')) {
    function getStatsForSuperadmin()
    {
        ...
        if ($clientPlan) {
            $clientInvoices = array_filter(array_column($clientPlan, 'is_invoiced'));

            $invoices_model = get_instance()->load->model('invoices_model');
            $allInvoice     = [];
            foreach ($clientInvoices as $firstInvoice) {
                $invoiceDetail    = get_instance()->invoices_model->get($firstInvoice);
                $recuringInvoices = get_instance()->invoices_model->get_invoice_recurring_invoices($firstInvoice);
                $planInvoices     = array_merge([$invoiceDetail], $recuringInvoices);
