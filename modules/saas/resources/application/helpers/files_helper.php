 ...
 * @param int    $permissions folder permissions
 *
 * @return bool
 */
...
function get_mime_class($mime)
{
    ...
    // Image
    if ('image' == $part1) {
        if (false !== strpos($part2, 'photoshop')) {
    ...
    // Applications
    elseif ('application' == $part1) {
        // Pdf
        if ('pdf' == $part2) {
            return 'mime mime-pdf';
        }
        // Ilustrator
        elseif ('illustrator' == $part2) {
            return 'mime mime-illustrator';
        }
        // Zip
        elseif ('zip' == $part2 || 'gzip' == $part2 || false !== strpos($part2, 'tar') || false !== strpos($part2, 'compressed')) {
            return 'mime mime-zip';
        }
        // PowerPoint
        elseif (false !== strpos($part2, 'powerpoint') || false !== strpos($part2, 'presentation')) {
            return 'mime mime-powerpoint ';
        }
        // Excel
        elseif (false !== strpos($part2, 'excel') || false !== strpos($part2, 'sheet')) {
            return 'mime mime-excel';
        }
        // Word
        elseif ('msword' == $part2 || 'rtf' == $part2 || false !== strpos($part2, 'document')) {
...
function bytesToSize($path, $filesize = '')
{
    ...
    if ($bytes > 0) {
        ...
        if (true === array_key_exists($unit, $units)) {
 ...
 * @param bool  $preview is preview image, will return url with thumb
 *
 * @return string
 */
...
function markdown_parse_preview($path)
{
    ...
    $Parsedown->setSafeMode('true' == $markDownSafeMode ? true : false);
...
function app_maybe_delete_old_temporary_files()
{
    ...
    foreach ($files as $file) {
        ...
        if ('index.html' == $file) {
            continue;
        } elseif (false !== strpos($file, '-upgrade-version-')) {
