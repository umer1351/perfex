...
function is_contact_email_verified($id = null)
{
    ...
    if (isset($GLOBALS['contact']) && $GLOBALS['contact']->id == $id) {
        return null !== $GLOBALS['contact']->email_verified_at;
    }
    ...
    return null !== $contact->email_verified_at;
}
...
{
    return 0 == total_rows(db_prefix().'emailtemplates', ['slug' => 'contact-verification-email', 'active' => 0]);
}
/**
...
function filter_client_visible_tabs($tabs, $id = '')
{
    ...
    foreach ($tabs as $key => $tab) {
        ...
        if ('profile' != $key && 'contacts' != $key && $appliedSettings) {
            if (array_key_exists($key, $visible) && false == $visible[$key]) {
                continue;
            }
        }

        if (null !== $customerProfileBadges) {
...
function app_init_customer_profile_tabs()
{
    ...
    $CI->app_tabs->add_customer_profile_tab('tickets', [
        ...
        'visible'  => ((1 == get_option('access_tickets_to_none_staff_members') && !is_staff_member()) || is_staff_member()),
        'position' => 75,
        'badge'    => [],
    ]);
...
function is_client_using_multiple_currencies($clientid = '', $table = null)
{
    ...
    if ($total_currencies_used > 1) {
        $retVal = true;
    } elseif (0 == $total_currencies_used || 1 == $total_currencies_used) {
        $retVal = false;
    }
...
function get_company_name($userid, $prevent_empty_company = false)
{
    ...
    $select = (false == $prevent_empty_company ? get_sql_select_client_company() : 'company');
...
function _check_vault_entries_visibility($entries)
{
    ...
    foreach ($entries as $entry) {
        if (1 != $entry['visibility']) {
            if (2 == $entry['visibility'] && !is_admin() && $entry['creator'] != get_staff_user_id()) {
                continue;
            } elseif (3 == $entry['visibility'] && $entry['creator'] != get_staff_user_id() && !is_admin()) {
                continue;
            }
        }
        $new[] = $entry;
...
function can_loggged_in_user_manage_contacts()
{
    ...
    if ((1 != $contact->is_primary) || (1 != get_option('allow_primary_contact_to_manage_other_contacts'))) {
        return false;
    }
...
function is_individual_client()
{
    return is_empty_customer_company(get_client_user_id())
        && 1 == total_rows(db_prefix().'contacts', ['userid' => get_client_user_id()]);
}
...
{
    if (null !== get_cookie('contact_language')) {
