...
{
    $recentSearches = get_staff_meta($staff_id ?: get_staff_user_id(), 'recent_searches');
...
function update_staff_recent_search_history($history, $staff_id = null)
{
    ...
    update_staff_meta($staff_id ?: get_staff_user_id(), 'recent_searches', json_encode($history));
...
function is_staff_member($staff_id = '')
{
    ...
    if ('' == $staff_id) {
        if (isset($GLOBALS['current_user'])) {
            return '0' === $GLOBALS['current_user']->is_not_staff;
        }
