...
function _perfex_upload_error($error)
{
    ...
    if (isset($uploadErrors[$error]) && 0 != $error) {
            ...
            if (!empty($tmpFilePath) && '' != $tmpFilePath) {
                ...
                if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                    ...
                    if (is_client_logged_in()) {
                        ...
                        $data['visible_to_customer'] = ('true' == $CI->input->post('visible_to_customer') ? 1 : 0);
                    }
 ...
 * @param int    $width    width of thumb
 * @param int    $height   height of thumb
 *
 * @return null
 */
