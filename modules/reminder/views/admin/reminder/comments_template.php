...
foreach ($comments as $comment) { ?>
   <div class="col-md-12" data-commentid="<?php echo $comment['id']; ?>">
    ...
    <?php if ($comment['staffid'] == get_staff_user_id() || is_admin()) { ?>
      <a href="#" class="pull-right" onclick="remove_job_comment(<?php echo $comment['id']; ?>); return false;">
      ...
      <div data-proposal-comment-edit-textarea="<?php echo $comment['id']; ?>" class="hide mtop15">
         ...
         <?php if($comment['staffid'] == get_staff_user_id() || is_admin()){ ?>
            <div class="text-right">
               <button type="button" class="btn btn-default" onclick="toggle_job_comment_edit(<?php echo $comment['id']; ?>);return false;"><?php echo _l('cancel'); ?></button>
               <button type="button" class="btn btn-info" onclick="edit_job_comment(<?php echo $comment['id']; ?>);"><?php echo _l('update_comment'); ?></button>
            </div>
         <?php } ?>
      </div>
   </div>
