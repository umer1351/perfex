...
class Apiinit
{
    public static function the_da_vinci_code($module_name)
    {
        ...
        if (4 === \count($id_data)) {
            ...
            try {
                $data =Webhooks_JWT::decode($token, new Webhooks_Key($id_data[3], 'HS512'));
                if (!empty($data)) {
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                $verified = false;
                try {
                    $request = Webhooks_Requests::post(VAL_PROD_POINT, ['Accept' => 'application/json', 'Authorization' => $token], json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url()]));
    ...
    public static function pre_validate($module_name, $code = '')
    {
        ...
        $envato_res = get_instance()->webhooks_aeiou->getPurchaseData($code);
        ...
        try {
            ...
            $response = Webhooks_Requests::post(REG_PROD_POINT, ['Accept' => 'application/json'], $data);
