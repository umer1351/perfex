...
foreach ($rResult as $aRow) {
    ...
    $options = icon_btn(WEBHOOKS_MODULE.'/webhook/'.$aRow['id'], 'pencil-square-o fa-solid fa-pencil', 'btn-default', ['data-toggle'=>'tooltip', 'data-title'=>_l('edit')]);
    $options .= icon_btn(WEBHOOKS_MODULE.'/delete_webhook/'.$aRow['id'], 'remove fa-solid fa-trash', 'btn-danger _delete', ['data-toggle'=>'tooltip', 'data-title'=>strtolower(_l('delete'))]);
    $options .= icon_btn(WEBHOOKS_MODULE . '/copy_webhook/' . $aRow['id'], 'fa-solid fa-copy', 'btn-info', ['data-toggle'=>'tooltip', 'data-title'=>_l('copy')]);
