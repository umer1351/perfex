...
class AdminController extends App_Controller
{
    public function __construct()
    {
        ...
        $this->maybe_activate_bundled_modules();
    ...
    /**
     * Perfect ERP - Auto-activate the addon modules that ship bundled with
     * this white-label build, so they work out of the box without requiring
     * a manual "Activate" click from Setup > Modules for each one.
     *
     * Safe by design:
     * - Uses the exact same App_modules::activate() path as the Modules UI,
     *   so each module's own install routine (tables, default settings,
     *   permissions) runs normally.
     * - Only attempts a module once: App_modules::activate() inserts the
     *   tblmodules row before running the module's install code, so on the
     *   next request it is already considered "installed" and is skipped -
     *   this also means an admin who deliberately deactivates one of these
     *   modules later will NOT have it silently re-activated.
     * - Wrapped per-module in try/catch so a problem with one module can
     *   never break the admin panel or take down the rest of the site;
     *   failures are written to the activity log instead.
     */
    private function maybe_activate_bundled_modules()
    {
        static $ran = false;
        ...
        foreach ($bundledModules as $moduleName) {
            try {
                if (!$this->app_modules->is_installed($moduleName) && !$this->app_modules->is_active($moduleName)) {
